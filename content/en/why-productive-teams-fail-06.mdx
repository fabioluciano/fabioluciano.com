...
<Callout type="info" title="What changed between 2014 and 2023">
...
**Cognitive Load:** Captures the mental effort required to operate — how much of the brain is spent on the real problem versus navigating accidental complexity.

<Callout type="warning" title="The choice wasn't scientific — it was strategic">

The 3 dimensions aren't "the only ones that matter." They're **the ones organizations can measure and act on without getting lost in complexity**. The framework traded comprehensiveness for actionability.
...
<Callout type="danger" title="Frameworks aren't neutral">

Every framework carries interests. DORA emerged from Google. SPACE emerged from Microsoft and GitHub research. DevEx has an explicit commercial connection. This doesn't invalidate them — but it reminds us that **what we choose to measure reflects what someone finds important to measure**.
...
<Mermaid
  ...
  chart={`graph TB
    ...
    CL["<b>Cognitive Load</b><br/><small>Mental effort</small>"]
...
<Callout type="warning" title="Flow is not isolation">
...
#### How feedback and cognitive load interact

Ambiguous feedback increases cognitive load. **Each obscure error message consumes mental energy.**
...
<Callout type="warning" title="Feedback is a system, not an isolated metric">
...
It was architecture that requires implicit knowledge to operate. Each microservice was created by a different person, with different patterns, without shared conventions. **Cognitive load is in the structure, not in the absence of text.**

#### How cognitive load accumulates silently

Cognitive load doesn't appear suddenly. It accumulates in small decisions, none absurd in isolation:

**Year 1:** "Let's use Kubernetes, everyone is using it"
- +10% cognitive load (learn K8s)

**Year 2:** "Let's add Istio for service mesh"
- +15% cognitive load (learn Istio + how it interacts with K8s)

**Year 3:** "We need Prometheus + Grafana for observability"
- +10% cognitive load (learn Prometheus query language)

**Year 4:** "Let's migrate to Terraform for infrastructure as code"
- +15% cognitive load (learn Terraform + HCL)

**Year 5:** "We're adding Vault for secrets management"
- +10% cognitive load (learn Vault + access policies)

**Result after 5 years:**
- No decision was stupid
- Each tool solves a real problem
- But the sum is **60% more cognitive load than at the start**
...
<Callout type="danger" title="The trap of growing complexity">

Each addition makes sense in isolation. The absurdity only becomes visible when trying to explain the entire system to someone new. Then someone says: "It's complex, but that's normal for systems this size." **No. It's complex because each generation added tools without questioning accumulated cognitive cost.**
...
#### When "simplifying" increases cognitive load

Not every attempt to reduce complexity works. Sometimes, "simplifications" worsen cognitive load:
...
<Callout type="warning" title="Simplicity is reducing load, not hiding complexity">
...
Flow, Feedback, and Cognitive Load aren't independent — **they form a system**.

**Virtuous cycle:**
- Low cognitive load → Developers understand the system quickly
...
- Clear system → Cognitive load stays low

**Vicious cycle:**
- High cognitive load → Developers don't understand the system
...
- Confusing system → Cognitive load increases further

**Feedback fits into this system:**

**Fast and clear feedback reduces cognitive load:**
...
**Slow and obscure feedback increases cognitive load:**
...
<Callout type="info" title="DevEx is a system, not a checklist">

You can't "improve DevEx" by optimizing one dimension while ignoring the others. Fast feedback doesn't help if cognitive load is too high to take advantage of it. Flow doesn't help if feedback is too slow to sustain continuity. **The three dimensions need to be addressed as an interdependent system.**
...
The DevEx framework focuses on individual and team experience — how developers feel while working, how much flow they achieve, how fast they receive feedback. **But it systematically ignores the organizational structures that produce this experience.**

**Concrete example:**

Team reports high cognitive load. DevEx survey confirms: developers spend 60% of their time navigating accidental complexity. Company invests in:
- Better documentation
- Faster tools
- Architecture training

**Six months later, cognitive load remains high.**
...
<Callout type="danger" title="Framework limits as organizational mirror">

When a framework ignores organizational dimensions, it allows organizations to treat political problems as if they were technical. "Let's improve DevEx" becomes a substitute for "let's redistribute power and change incentives". One is palatable. The other is threatening.
...
- High cognitive load: confusing architecture
...
**DevEx became a check-box. Something to show in all-hands and recruitment.** The framework was instrumentalized to perform care without actually exercising it.

<Callout type="warning" title="Surveys without power to change aren't DevEx">
...
Productivity frameworks are never neutral. They carry interests — from who created them, who funds them, who implements them. DevEx is no exception.
...
<Callout type="danger" title="Who controls the definition, controls the outcome">

If "improving DevEx" only means optimizing tools and processes without touching workload, autonomy, or pressure, the framework becomes an instrument for maintaining the status quo disguised as progressive improvement.
...
<Callout type="danger" title="Optimization without limits is exhaustion">
...
DevEx doesn't distribute benefits equally. And frameworks that ignore this perpetuate inequalities under a veneer of "improvement for all".
...
<Callout type="warning" title="DevEx doesn't substitute purpose">
...
DevEx measures flow, feedback, and cognitive load. But doesn't ask: **productivity for what?**
...
**The framework doesn't answer "what is this productivity for?"** It assumes productivity is intrinsically good. But productivity applied to valueless product, or sustained by exploitation, or used to maintain status quo — **isn't achievement. It's a problem.**

<Callout type="danger" title="Technical frameworks don't substitute political questions">

DevEx offers lenses to see experience problems. But **it doesn't offer criteria to decide when experience matters more than other things**, or when improving experience serves dubious purposes. This requires judgment that no framework provides.
