...
É aqui que Developer Experience (DevEx) entra como lente alternativa: em vez de perguntar 'quanto produzimos?', pergunta 'quanto custa cognitivamente produzir isso?'. Trataremos DevEx não como conforto, mas como economia cognitiva.
...
<Mermaid
  client:load
  chart={`flowchart TD
    A([Escolha do que medir]) --> B[Métrica se torna visível]
    B --> C[Comportamento se adapta]
    C --> D[Métrica 'melhora']
    D --> E{Otimização genuína?}
    E -->|Sim| F[Resultado real melhora]
    E -->|Não| G[Consequências não-intencionais]
    F --> H[Nova métrica é proposta]
    G --> H
    H --> A
  ...
  caption="Métricas não apenas medem realidade — elas a transformam. O ciclo de visibilidade mostra como a escolha do que medir molda o que times priorizam e como se comportam."
/>
