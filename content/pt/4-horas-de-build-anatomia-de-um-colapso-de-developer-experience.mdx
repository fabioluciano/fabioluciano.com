...
description: "Quando milhares de testes, centenas de GB de RAM e uma cultura tóxica se encontram: a anatomia completa de um desastre de developer experience"
...
toc: true
comments: true
translationSlug: 4-hours-build-anatomy-devex-collapse
---
...
<Mermaid
  ...
  caption="As 13 camadas entre você e o código que está testando. Cada camada adiciona complexidade de debug exponencialmente."
/>
...
**3h59min:** Surefire gerando relatórios XML...
...
<Mermaid
  client:load
  chart={`graph LR
    A[0:00 - Build inicia ✅] --> B[1:00 - 25% passando ✅]
    B --> C[2:00 - 50% passando ✅]
    C --> D[3:00 - 75% passando ✅]
    D --> E[3:58 - TODOS passaram! 🎉]
    ...
    style E fill:#51cf66
    style G fill:#ff6b6b`}
  caption="O ciclo do sofrimento: 4 horas de espera, tudo verde, falha na última etapa, retry automático."
/>
...
<Mermaid
  ...
  chart={`graph TB
    ...
    INFRA["<b>Infraestrutura</b><br/>Não é problema de recursos<br/>~7.000 testes é absurdo<br/>Otimizem o código"]

    PROBLEMA["<b>O PROBLEMA</b><br/>4h build<br/>Consumo desenfreado de recursos<br/>Logs impossíveis"]
    ...
    SOLUCAO["<b>SOLUÇÃO?</b><br/>❌ Nenhuma<br/>Ego > Colaboração"]

    DEV -.->|Não chegam| SOLUCAO
    INFRA -.->|Não chegam| SOLUCAO

    style PROBLEMA fill:#ff6b6b
    style SOLUCAO fill:#ffd43b`}
  caption="O impasse organizacional: Dev culpa Infra, Infra culpa Dev, ninguém assume ownership."
/>
...
<Mermaid
  ...
  chart={`pie
    ...
    "Null Checks" : 200
    "Bean Validation" : 70`}
  caption="50% dos testes testavam frameworks, compiladores ou eram duplicados. Apenas 27% testavam lógica de negócio real."
/>
...
Você pode ter Deployment Frequency "boa" no dashboard enquanto desenvolvedores evitam fazer mudanças porque cada uma significa um ciclo de feedback interminável. A métrica está verde. O time está em burnout.
...
Agora, vamos ver como essas três verdades se manifestam em cada framework de produtividade. Esta série explora DORA<FootnoteRef client:load id={1} /> (entrega), SPACE<FootnoteRef client:load id={2} /> (multidimensional) e DevEx<FootnoteRef client:load id={3} /> (experiência do desenvolvedor).

### Métricas DORA: quando as métricas mentem

<Tooltip client:load term="DORA Metrics" definition="Conjunto de 4 métricas que avaliam a capacidade de entrega de software: frequência de deploy, tempo de entrega, tempo de recuperação e taxa de falhas em mudanças.">As métricas DORA</Tooltip> são desenhadas para medir capacidade de entrega. Mas há um detalhe importante: **havia implantação em ambiente de testes**, não produção.
...
- 4 horas de ciclo de feedback para cada tentativa de build
...
**Tempo de Entrega para Mudanças: ❌ 2-3 DIAS (às vezes SEMANAS)**
...
**GitFlow + liberações em batch** multiplicavam o tempo de entrega. Uma mudança simples ficava travada esperando o próximo ciclo de release. Às vezes semanas.
...
Deployment Frequency não distingue entre "deploys confiáveis" e "deploys que só passam no retry". Tempo de Entrega não captura os 3 builds que falharam antes do sucesso. Change Failure Rate de 30% parece razoável, mas esconde que **60% dos builds falhavam na primeira tentativa** antes mesmo de chegar aos testadores.
...
<Tooltip client:load term="SPACE Framework" definition="Framework de 5 dimensões para medir produtividade: Satisfaction, Performance, Activity, Communication e Efficiency.">SPACE</Tooltip> expande a análise para múltiplas dimensões. E cada uma delas estava destruída:
...
**A - Activity: ❌ 4h de ciclo de feedback**
...
<ProsCons
  ...
  consTitle="O que SPACE não resolveria"
  context="SPACE como diagnóstico"
  variant="warning"
  pros={[
    { text: "Satisfação no chão — visível em questionários", emphasis: true },
  ...
  cons={[
    ...
    { text: "Quem tem autoridade para mudar (decisão política)", emphasis: true },
...
<Tooltip client:load term="DevEx" definition="Framework de pesquisa (2023) que define três dimensões para avaliar a experiência do desenvolvedor: estado de fluxo, ciclos de feedback e carga cognitiva.">DevEx</Tooltip> olha para as três dimensões centrais de experiência: Fluxo, Feedback e Carga Cognitiva.
...
<Mermaid
  client:load
  chart={`graph TB
    F["<b>Fluxo</b><br/><small>DESTRUÍDO</small><br/>4h de feedback = fluxo impossível"]
    FB["<b>Feedback</b><br/><small>AUSENTE</small><br/>Logs impossíveis, ferramentas negadas"]
    CL["<b>Carga Cognitiva</b><br/><small>MÁXIMA</small><br/>13 camadas, ponte Dev↔Infra, conhecimento tribal"]

    F -.->|"Sem fluxo<br/>decisões apressadas"| CL
    FB -.->|"Sem feedback<br/>debug às cegas"| CL
  ...
  caption="As 3 dimensões do DevEx formam um ciclo vicioso: sem fluxo, sem feedback e carga cognitiva insustentável."
/>
...
O **DX Core 4**<FootnoteRef id={4} /> é um framework mais recente que expande o DevEx original para 4 dimensões interdependentes: **Flow**, **Feedback**, **Cognitive Load** e **Alignment**.
...
<Mermaid
  ...
  chart={`graph TB
    subgraph DX["<b>DX Core 4 no Build de 4 Horas</b>"]
        F["<b>Flow</b><br/><small>DESTRUÍDO</small><br/>4h de ciclo de feedback<br/>Contexto perdido"]
        FB["<b>Feedback</b><br/><small>AUSENTE</small><br/>Logs impossíveis<br/>Erro ambíguo"]
        CL["<b>Cognitive Load</b><br/><small>MÁXIMA</small><br/>13 camadas<br/>Conhecimento tribal"]
        AL["<b>Alignment</b><br/><small>QUEBRADO</small><br/>Dev vs Infra<br/>Ego > Colaboração"]
    end
  ...
  caption="As 4 dimensões do DX Core 4 formam um ciclo de reforço negativo: cada problema amplifica os outros."
/>
...
Pior ainda: múltiplas tentativas. Build falha, você ajusta uma linha, roda de novo. Mesma espera. No fim do dia, 2-3 tentativas consumiram 8-12 horas de tempo real para validar uma mudança que deveria levar minutos. Transferências constantes. Contexto perdido. Carga mental crescente.
...
<Callout type="danger" title="O ciclo de reforço negativo">
...
| 50% desperdício identificado | Parcial | - | ✅ | ✅ |
...
DORA capturou **sintomas** — tempo de entrega alto, change failure rate. SPACE identificou a **guerra entre times**. DevEx mostrou a **destruição cognitiva**. Mas só DX Core 4 mostrou o **ciclo vicioso completo** onde cada problema amplifica os outros.

### O que cada framework proporia

| Framework | Solução Proposta | Custo Estimado | Impacto Esperado |
|-----------|------------------|----------------|------------------|
| **DORA** | Staged pipeline (smoke→unit→e2e)<br/>Simplificar GitFlow → trunk-based<br/>Reduzir tempo de entrega com builds incrementais | Médio<br/>2-3 meses de trabalho | Tempo de Entrega: 2-3 dias→4-8h<br/>Deployment Frequency: 2-3/sem→diário<br/>MTTR: 6h→2h |
| **SPACE** | Ferramentas de debug (kubectl, Grafana, Loki)<br/>Estrutura de colaboração entre times<br/>Autonomia | Baixo (ferramentas)<br/>Alto (mudança cultural) | Satisfaction: 2→7<br/>Collaboration: guerra→parceria<br/>Efficiency: fricção reduzida |
| **DevEx** | Eliminar 50% dos testes desnecessários<br/>Logs estruturados (JSON)<br/>Feedback rápido (build &lt;10min) | Baixo (delete testes)<br/>Médio (estruturar logs) | Flow: 4h→30min de feedback<br/>Carga Cognitiva: redução 40%<br/>Feedback: claro e acionável |
| **DX Core 4** | Eliminar Extraneous Load (testes frameworks, camadas)<br/>Estabelecer acordo entre times (Alignment)<br/>Criar estrutura de decisão clara | Médio<br/>Requer liderança forte | Cognitive Load: -70%<br/>Alignment: conflito→colaboração<br/>Flow: 4h→1h<br/>Feedback: ambíguo→claro |
...
- Tempo de entrega: 2-3 dias → 4-8 horas (80-90% redução)
...
**Problema:** GitFlow + batch releases = tempo de entrega inflado artificialmente.
...
**Impacto:**
- Tempo de entrega: 2-3 dias → 4-8 horas
...
**Custo oculto (o pior):** Burnout inevitável. Moral no chão com satisfação 2/10. Velocidade decrescente — tempo de entrega aumentando mês a mês. Dívida técnica acumulando porque ninguém quer refatorar um sistema que já é pesadelo de trabalhar.
...
<FootnoteList>
  <FootnoteItem id={1}>
    Forsgren, Nicole; Humble, Jez; Kim, Gene. **[Accelerate: The Science of Lean Software and DevOps](https://www.oreilly.com/library/view/accelerate/9781457191435/)**. IT Revolution Press, 2018. Apresenta as 4 métricas DORA (deployment frequency, tempo de entrega, MTTR, change failure rate) e as 24 capacidades técnicas e organizacionais que sustentam alta performance em entrega de software.
  </FootnoteItem>
  <FootnoteItem id={2}>
    Storey, Margaret-Anne; Zimmermann, Thomas; et al. **[The SPACE of Developer Productivity](https://queue.acm.org/detail.cfm?id=3454124)**. ACM Queue, 2021. Framework que propõe 5 dimensões para medir produtividade de desenvolvedores: Satisfaction, Performance, Activity, Communication e Efficiency.
  </FootnoteItem>
  <FootnoteItem id={3}>
    Noda, Abi; et al. **[DevEx: What Actually Drives Productivity](https://queue.acm.org/detail.cfm?id=3595878)**. ACM Queue, 2023. Framework baseado em 3 dimensões (fluxo, feedback, carga cognitiva) para avaliar e melhorar developer experience de forma sistemática.
  </FootnoteItem>
  <FootnoteItem id={4}>
    Forsgren, Nicole; Storey, Margaret-Anne; et al. **[DevEx in Action: A Study of Its Tangible Impacts](https://queue.acm.org/detail.cfm?id=3639443)**. ACM Queue, 2024. Expansão do framework DevEx para 4 dimensões: Flow, Feedback, Cognitive Load e Alignment. Foca em intervenções pragmáticas e interdependências entre as dimensões.
  </FootnoteItem>
</FootnoteList>
