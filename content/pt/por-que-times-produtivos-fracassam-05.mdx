...
Depois de olhar para o DORA, fica tentador acreditar que produtividade em software pode ser entendida — e melhorada — observando a pipeline. Em muitos aspectos, isso é verdade. Sistemas previsíveis, com baixo custo de mudança, criam condições reais para que o trabalho avance.
...
#### Por que satisfação importa tecnicamente

Satisfação costuma ser tratada como algo "soft", secundário, quase um luxo. O SPACE inverte essa lógica. Ele trata satisfação e bem-estar como **variáveis estruturais**, não como bônus emocionais. Um desenvolvedor satisfeito não é apenas alguém mais feliz; é alguém com maior capacidade de concentração, julgamento e aprendizado.

<Callout type="info" title="O argumento técnico">
  Satisfação não é sobre fazer pessoas felizes. É sobre reconhecer que **capacidade cognitiva é recurso finito** — e que sistemas que desgastam pessoas sistematicamente reduzem essa capacidade.
</Callout>
...
#### O que acontece quando ignoramos satisfação

Ignorar esse eixo não torna o sistema mais objetivo — apenas o torna cego para o desgaste acumulado. O sistema continua funcionando até que não funciona mais. Rotatividade, burnout e decisões defensivas são sintomas tardios de satisfação ignorada.

#### Como medir satisfação (sem pesquisas vazias)

A tentação é reduzir satisfação a pesquisas de clima organizacional — aquelas com perguntas genéricas que ninguém leva a sério. SPACE sugere algo diferente: medir satisfação **através de proxies comportamentais**.
...
#### Satisfação como indicador antecedente
...
<ProsCons
  client:load
  prosTitle="Satisfação alta indica"
  consTitle="Satisfação baixa indica"
  context="Satisfação como sinal de saúde sistêmica"
  variant="balanced"
  pros={[
    "Sistema sustentável",
    "Capacidade de absorver pressão",
    "Decisões técnicas de qualidade",
    "Colaboração genuína"
  ]}
  cons={[
    { text: "Sistema funcionando sob estresse", emphasis: true },
    "Reservas cognitivas sendo consumidas",
    { text: "Qualidade silenciosamente deteriorando", emphasis: true },
    "Colaboração se tornando política"
  ]}
/>
...
<Callout type="tip" title="A distinção crucial">
  Entregar rápido não é o mesmo que entregar bem. Um sistema pode gerar muita entrega técnica enquanto falha consistentemente em produzir resultado significativo.
</Callout>

#### A confusão entre velocidade e performance

Organizações frequentemente tratam velocidade como sinônimo de performance. Mais commits por dia, mais tickets fechados por iteração, menos tempo entre requisição e entrega. Esses números são fáceis de coletar e satisfazem a necessidade gerencial por progresso visível.

Mas velocidade mede **vazão do sistema**. Performance, no sentido SPACE, mede **qualidade do resultado**. São dimensões diferentes — e otimizar uma não garante a outra.
...
A preferência por velocidade não é acidente. Ela existe porque **velocidade é fácil de medir e performance é difícil de definir**.

Quanto tempo leva para saber se uma decisão técnica foi boa? Meses, às vezes anos. Quanto tempo para saber se um ticket foi fechado? Segundos. A assimetria temporal cria um viés sistemático: o que é mensurável agora ganha atenção; o que só se revela depois é ignorado.

<Callout type="warning" title="O custo invisível da velocidade">
  Um time pode fechar 50 tickets por iteração enquanto acumula débito técnico que triplicará o custo de manutenção em seis meses. Nos painéis de hoje, esse time parece excelente. Nos problemas de amanhã, ele será a causa.
</Callout>
...
Aqui está o problema: performance real é difícil de medir diretamente. Você pode contar commits, mas não pode contar "decisões técnicas que envelheceram bem". Pode medir tempo de entrega, mas não pode medir "problemas que não aconteceram porque alguém pensou melhor".
...
<ProsCons
  client:load
  prosTitle="Sinais de performance alta"
  consTitle="Sinais de performance baixa"
  context="Como identificar performance real além da velocidade"
  variant="balanced"
  pros={[
    "Bugs raramente voltam como regressão",
    "Código novo integra bem com existente",
    "Decisões técnicas se sustentam no tempo",
    "Usuários percebem valor nas entregas"
  ]}
  cons={[
    { text: "Retrabalho constante nos mesmos módulos", emphasis: true },
    "Funcionalidades entregues que ninguém usa",
    { text: "Débito técnico crescendo silenciosamente", emphasis: true },
    "Soluções que criam mais problemas do que resolvem"
  ]}
/>
...
<ProsCons
  client:load
  prosTitle="O que atividade alta pode significar"
  consTitle="O que atividade alta realmente pode ser"
  context="A natureza traiçoeira da dimensão Activity"
  variant="warning"
  pros={[
  ...
  cons={[
    { text: "Retrabalho constante", emphasis: true },
    "Interrupções fragmentando o trabalho",
    { text: "Falta de foco e clareza", emphasis: true }
  ]}
/>

SPACE não ignora atividade, mas a trata como **sinal fraco**. Atividade sem contexto é ruído. O modelo não diz "não meça". Ele diz: **não confunda movimento com avanço**.
...
A preferência por métricas de atividade revela um viés cognitivo profundo: **se podemos contar, devemos estar entendendo**. Mas contar não é entender. É apenas uma forma específica — e limitada — de representar realidade.

Commits contados não dizem nada sobre a qualidade do código. Tickets fechados não dizem nada sobre o valor entregue. Horas conectadas não dizem nada sobre o trabalho real feito.

<Callout type="warning" title="O que atividade não captura">
  Pensar. Planejar. Discutir. Aprender. Revisar com cuidado. Esperar pela informação certa antes de agir. Todas essas atividades são invisíveis para métricas de contagem — e todas são essenciais para trabalho de qualidade.
</Callout>
...
- **Como contexto para as demais dimensões do SPACE**: Muita atividade + satisfação baixa = sinal de desgaste
- **Como referência individual**: Mudanças bruscas no padrão de atividade merecem atenção

O erro não é medir atividade. É tratá-la como **indicador principal** quando ela deveria ser **indicador complementar**.

<ProsCons
  client:load
  prosTitle="Uso correto de atividade"
  consTitle="Uso incorreto de atividade"
  context="Como interpretar métricas de atividade"
  variant="balanced"
  pros={[
    "Comparar padrões ao longo do tempo",
    "Identificar anomalias que merecem investigação",
    "Cruzar com outras dimensões",
    "Usar como ponto de partida para conversas"
  ]}
  cons={[
    { text: "Ranquear times por atividade", emphasis: true },
    "Usar como meta de performance",
    { text: "Assumir que mais é melhor", emphasis: true },
    "Ignorar o contexto do número"
  ]}
/>

#### A pergunta que atividade não responde

A pergunta fundamental que atividade não consegue responder é: **esse movimento está nos levando para onde queremos ir?**

Um time pode estar extremamente ativo enquanto caminha em círculos. Pode mostrar números impressionantes de tickets, commits e deploys enquanto o produto estagna, a dívida técnica cresce e os usuários continuam insatisfeitos.

Atividade mede **quanto** o sistema se move. Não mede **para onde**.
...
<Callout type="warning" title="Custo invisível">
  Fricção social também impacta produtividade. Reuniões excessivas, canais ruidosos e decisões mal distribuídas consomem energia cognitiva da mesma forma que sistemas mal projetados.
</Callout>
...
Quando o DORA mostra números ruins, a conversa tende a ser direta: a pipeline está lenta, as implantações estão quebrando, precisamos melhorar. É desconfortável, mas **decisivo**. Há um alvo claro.
...
<Callout type="danger" title="A escolha que ninguém quer admitir">
  SPACE não falha por ser mal compreendido. **Ele falha por ser bem compreendido demais.**

  Organizações resistem a ele não porque é complexo, mas porque **torna visível aquilo que preferiam manter implícito**: que produtividade tem custo, que otimização exige sacrifício, e que essas escolhas são políticas — não técnicas.
</Callout>
