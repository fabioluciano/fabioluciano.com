...
import { Callout, Timeline, TimelineEvent, Steps, Step, Accordion, AccordionItem, LinkCard, LinkCardGrid, Mermaid } from '@/components/mdx';
...
Trocar de tecnologia sem mudar o modelo mental é uma armadilha comum quando falamos de automação. Você pode ter a pipeline mais sofisticado do mundo, mas se o problema fundamental não foi endereçado, você apenas automatizou a ineficiência.
</Callout>
...
<Mermaid
  ...
  chart={`flowchart TB
    subgraph sources[Fontes de Dados]
        common[common.yaml<br/>dados compartilhados]
        en[resume.en.yaml<br/>traduções]
        ptbr[resume.ptbr.yaml<br/>traduções]
    end

    schema[JSON Resume Schema<br/>validação]

    typst[Typst Engine<br/>data-loader.typ merge]

    subgraph templates[Templates]
        t_full[resume-full.typ]
        t_onepage[resume-onepage.typ]
        t_html[resume-html.typ]
    end

    subgraph outputs_en[Saídas EN]
        out_full_en[EN resume.pdf]
        out_onepage_en[EN resume-onepage.pdf]
        out_html_en[EN index.html]
    end

    subgraph outputs_pt[Saídas PT]
        out_full_pt[PT resume.pdf]
        out_onepage_pt[PT resume-onepage.pdf]
        out_html_pt[PT index.html]
    end
    ...
    t_onepage --> out_onepage_en
    t_onepage --> out_onepage_pt
...
O site em HTML e os PDFs — tanto a versão completa quanto a versão condensada em uma única página — são gerados a partir da mesma fonte de verdade. Se algo quebra, a pipeline acusa. Se algo muda, a atualização é trivial.
