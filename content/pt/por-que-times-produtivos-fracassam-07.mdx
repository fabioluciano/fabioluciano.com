...
Depois de DORA, SPACE e DevEx, organizações chegam a um estado específico. O <Tooltip client:load term="DX Core 4" definition="Framework que identifica quatro dimensões centrais da Developer Experience: feedback loops, cognitive load, flow state e developer satisfaction.">DX Core 4</Tooltip> nasce exatamente nesse ponto de saturação cognitiva. Não como um novo modelo explicativo, mas como uma **redução deliberada da complexidade**.
...
- **DORA** mede resultados (<Tooltip client:load term="lead time" definition="Tempo desde o commit do código até a entrega em produção. Métrica DORA que mede velocidade de entrega.">lead time</Tooltip>, deployment frequency). **DX Core 4** investiga as condições que produzem esses resultados.
...
- **Throughput** (volume de trabalho completado por unidade de tempo)
...
Você pode ter **<Tooltip client:load term="lead time" definition="Tempo desde o commit do código até a entrega em produção. Métrica DORA que mede velocidade de entrega.">lead time</Tooltip> baixo** (métrica DORA) mas **fluxo degradado** (experiência DX Core 4).

**Como isso acontece:**

- Lead time mede agregado (média do time/mês)
- Fluxo mede vivência individual cotidiana

**Exemplo prático:**

Time tem lead time de 2 horas (excelente!). Mas consegue isso porque:
- 70% das mudanças são triviais e fluem rápido
- 30% das mudanças são bloqueadas por dias

**Resultado:** Lead time agregado é ótimo. Experiência de fluxo é miserável para quem trabalha nas mudanças complexas.
...
**O que DORA ou SPACE mostrariam?** Métricas normais. Lead time ok. Satisfação ok (dos que sobreviveram).
...
- Produto pressiona por features rápidas
...
**Resultado:** Lead time aumenta (porque cada lado puxa para direção diferente). Satisfação cai (porque todos sentem que "ninguém entende a importância" do que fazem). Valor real entregue diminui.
...
<ProsCons
  ...
  cons={[
    { text: "Tratar como compliance checklist", emphasis: true },
...
**O que aconteceu:** DX Core 4 virou checklist. Em vez de ferramenta de pensamento, virou auditoria de compliance. **Temos os 4 eixos cobertos, logo estamos bem.**
...
- **Restrições externas:** Regulação, compliance, contratos legados
...
### Limitação 2: Compliance e regulação

**Cenário:** Fintech regulada. Compliance exige auditoria manual de toda mudança, aprovações múltiplas, documentação extensiva.

**DX Core 4 diz:** "Melhore feedback loops! Reduza tempo de espera!"
